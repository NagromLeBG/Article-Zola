...
La NMF est la première des deux méthodes utilisées pour identifier les thèmes dans les romans de Zola.
...
Afin d'orienter ce choix, une méthode du coude a d'abord été utilisée\footnote{Code visible en annexe}. Elle consiste à entraîner plusieurs modèles NMF avec différents nombres de topics, puis à observer l'évolution de l'erreur de reconstruction. L'idée est de repérer un point à partir duquel l'ajout de nouveaux topics améliore moins fortement la qualité de reconstruction.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{article zola/figure/methode_du_coude_nmf.png}
    \caption{Méthode du coude pour le choix du nombre de topics dans la NMF}
    \label{fig:methode_du_coude_nmf}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{article zola/figure/silhouette.png}
